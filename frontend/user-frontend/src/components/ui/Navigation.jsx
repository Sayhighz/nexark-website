...
export function List({ as = "div", children, className, ...props }) {
  const Component = as;
...
export function Item({ onActivated = () => {}, active = false, as = "div", children, className, ...props }) {
  const Component = as;
  ...
  useEffect(() => {
    // set specified default index (fallback to 0)
    const initialIndex = rootContext?.defaultIndex ?? 0;
    if (rootContext.activeItem.index === -1 && index === initialIndex) {
      setActive(false)
    }
  }, [rootContext.isMounted, index, rootContext.activeItem.index, rootContext.defaultIndex])

  // respond to external active prop changes to animate highlight
  useEffect(() => {
    if (active && rootContext.activeItem.index !== index) {
      setActive(false)
    }
  }, [active, index, rootContext.activeItem.index])
